...
if menu == "Funcionário":
    ...
    if cpf_input:
        ...
        try:
            ...
            st.error("Erro de conexão com o banco de dados. Tente novamente em instantes.")
            func = None
        ...
        if func:
            ...
            st.error("CPF não encontrado ou falha de conexão. Certifique-se de digitar com pontos e traços.")
    ...
    if empresas:
        ...
        if st.button("CARREGAR DADOS ATUALIZADOS"):
            try:
                ...
                res = None
                st.error("Erro ao conectar com o Supabase para carregar os dados.")
            ...
            if res and res.data:
                ...
                # --- LEGENDA COMPACTA E PRÓXIMA ABAIXO DO GRÁFICO ---
                st.markdown("""
                <div style="display: flex; justify-content: flex-start; gap: 30px; font-size: 0.8rem; padding: 10px; border-top: 1px solid #ddd; color: #555;">
                ...
                st.error("Nenhum dado encontrado para esta empresa ou erro de conexão.")
    else:
        st.error("Não foi possível carregar a lista de empresas. Verifique sua conexão com a internet ou o painel do Supabase.")import streamlit as st
from supabase import create_client
import pandas as pd
import plotly.express as px
...
# --- MODO GESTOR ---
    ...
    if empresas:
        ...
        st.error("Não foi possível carregar a lista de empresas. Verifique sua conexão com a internet ou o painel do Supabase.")
